...
# Initialize Firebase
cred = credentials.Certificate("E:/01fe22bee409/OneDrive/Desktop/stream/realsense-monitoring-frontend/src/components/serviceAccountKey.json")
...
def generate_advice(data, problems):
    ...
    if not any("perfect" in p for p in problems):
        advice.append("Consider soil amendments or greenhouse adjustments")
    return advice

def process_data_with_flask(data):
    print("\n=== NEW ANALYSIS ===")
    ...
    clean_data = {
        ...
        'P': data.get('soilP', 0),
        'K': data.get('soilK', 0),
        'EC': data.get('soilEC', 0),
        'pH': data.get('soilPH', 7),
        'lux': data.get('lux', 0),
        'temp': data.get('airTemp', 25),
        'humidity': data.get('humidity', 50),
        'moisture': data.get('soilMoisture', 1.0)
    }
...
if __name__ == "__main__":
    print("🚜 Real-time Crop Recommendation System Started")
    print("🌐 Flask server starting...")
    print("🔁 Listening for latest updates...")
    ...
    # Start Firebase listener in main thread
    setup_listener()
